...
10. Код магазина со всеми функциями п5,6,7,8,9

#### struct

1. Взять код из задачи 10 на map и сделать на структурах: пользователь, счет, товар, прейскурант, магазин, заказ. Без глобальных объектов. Использовать: struct, конструкторы и методы.
2. Взять код из 1 и сделать разные виды товаров и пользователей: premium пользователи у которых 5% скидка на обычные товары и 20% на premium товары; premium товары, которые продаются обычным пользователям с 50% наценкой.
3. Взять код из 2 и добавить наборы товаров в магазин: обычный или premium товар становится основным в наборе и к нему добавляется еще один только обычный товар. На набор предоставляется любая скидка от 1 до 99%, размер скидки выставляется при создании набора. Нужен списоб изменения размера скидки после создания набора.
4. Новый вид наборов: товар+один бесплатный пробник. Новый вид товаров - пробник. Цены не имеет, нельзя пукить отдельно, без набора.
5. Сделать экспорт и импорт данных магазина в файл и из файла. Чтобы можно было выгрузить все данные по товарам, пользователям в файл, а потом восстановить состояние магазина, прочитав этот файл.

#### concurrency

1. Взять магазин из *struct*. Написать бенчмарки
2. Методы магазина сделать http json rpc вызывами
3. Написать бенчмарки на json rpc. Профилирование.
4. Настроить ci. Проверять разницу в бенчмарках для pr. Если падение больше 15%, то блокировать задачу.
5. Запуск тестов с флагом race
4. На все методы добавить timeout. Для начала константный для всех, скажем, в секунду.
5. Распараллелить import, export.
6. Проверить бенчмарки ещё раз. Профилирование.
7. Попробовать jsoniter и еще раз бенчмарки

#### web
1. Регистрация пользователя через форму
2. Регистрация пользователя через oauth2
3. Авторизация пользователя
4. В личном кабинете показывать ранее купленные товары в виде отдельных заказов + время заказа + общая сумма
5. Добавить fasthttp, проверить бенчмарки, профилирование в виде графа, таблицы и диаграмы.
6. Коды ошибок для json rpc
7. Пробросить context
8. Добавить сбор метрик. Prometheus+grahana
9. Добавить запись логов. Logrus
8. Сделать ограничение на количество запросов: на просмотр товаров 5k rps, на добавление/изменение 1k rps, на заказ 4k rps, на импорт 1 одновременный запрос, на экспорт - 10 rps, экспорт должен блоктроваться импортом. Если превысили лимит, то ждём. Это soft limit. Надо еще hard limit, если достигаем его, то сразу отдаем ошибку пользователю. Hard limits: на просмотр товаров 10k rps, на добавление/изменение 5k rps, на заказ 10k rps, на импорт 100, на экспорт - 1000 rps
9. Добавить таймауты на запросы: на просмотр товаров 1ms, на добавление/изменение 1s, на заказ 10ms, на импорт 1h, на экспорт 5m, экспорт должен блоктроваться импортом.
10. Сделать профилирование.
11. Запаковать сервис в docker контейнер.

#### платежи
1. Добавить платежи через visa через gateway
2. Добавить платежи в bitcoin и eth

#### обработка
1. Добавить добавление картинок для товаров
2. Поддерживаемые форматы jpeg, png, gif без анимации
3. Минимальное разрешение по любой стороне 150px
4. Картинки добавляются ссылками, которые затем загружаются на сервер. Загрузка не должна блокировать импорт. В ответ пользователю отдаем id его щапроса, по которому он может проверять статус загрузки товара
5. Картинки конвертируем в три формата jpg: 150*150, 300*300, 450*450
6. Soft limit 1000
7. Hard limit 5000
8. Timeout 2s
9. Настройки таймаутов и лимитов можно читать из файла или переменных окружения. Приоритет должен задаваться в приложении
