...
- fmt, math, os, path, rand, binary, srand, tensing, runtime
...
8. Сделать текстовый калькулятор. Пользователь вводит предложения вида "5 плюс 6', а программа выдаёт результат вычисления. Реализовать сложение, вычитание, умножение, деление. Для начала только для двух чисел.
9. К 7 добавить, чтобы хранились пользовательские аккаунты со счетом типа "вася: 300р, петя: 30000000р". И перед оформлением заказа, но после его рассчёта мы проверяли, а есть ли деньги у пользователя, и если есть, то списывали сумму заказа.
10. Есть мапа аккаунтов и счетов, как описано в 9. Надо вывести ее в отсортированном виде с сортировкой: по имени в алфавитном порядке, по имени в обратном порядке, по количеству денег по убыванию.
